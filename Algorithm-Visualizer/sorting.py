...
class Sortings:
        ...
        for _ in range(len(data)-1):
            for j in range(len(data)-1):
                if data[j] > data[j+1]:
                    ...
                    drawData(data, ['#ff9900' if x == j or x == j+1 else 'indigo' for x in range(len(data))] )
                    time.sleep(timeTick)
        for i in range(2):
    ...
    def selection_sort(self,data, drawData, timeTick):
        for i in range(len(data)-1):
            ...
            drawData(data, ['#ff9900' if x == min_idx or x == i else 'indigo' for x in range(len(data))] )
            time.sleep(timeTick)
        for i in range(2):
            winsound.Beep(2500, 300)
            time.sleep(0.05)
    ...
    def getColorArray(self,dataLen, left, right, border, currIdx, isSwaping=False):
        ...
        for i in range(dataLen):
            ...
            if i >= left and i <= right:
                ...
                colorArray.append('indigo') #white

            if i == right:
                colorArray[i] = '#996633' #brown
            elif i == border:
                colorArray[i] = 'red'
            elif i == currIdx:
                colorArray[i] = 'yellow'
    ...
    def merge(self,data, left, middle, right, drawData, timeTick):
        ...
        drawData(data, ["#ff9900" if x >= left and x <=
                        right else "indigo" for x in range(len(data))])
        time.sleep(timeTick)
